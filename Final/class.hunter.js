...
module.exports = class Hunter extends Mother {
    ...
    hunt() {
        this.huntmulti++;
        if (weather == "Winter" ) {
            var asd = Math.floor(Math.random() * this.yntrelVandak2(2).length);
            var gishPoint = this.yntrelVandak2(2)[asd]
            if (this.huntmulti >= 7 && gishPoint && this.extraction < 5) {
                matrix[gishPoint[1]][gishPoint[0]] = 0;
                for (var i in xotakerarr) {
                    if (xotakerarr[i].x == gishPoint[0] && xotakerarr[i].y == gishPoint[1]) {
